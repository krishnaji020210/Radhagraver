...
def get_buttons(page, total, per_page=5):
    ...
    buttons = []
    if page > 0:
        buttons.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"waifus_prev_{page-1}"))
    if page < max_pages:
        buttons.append(InlineKeyboardButton("Next ➡️", callback_data=f"waifus_next_{page+1}"))
    buttons.append(InlineKeyboardButton("✖️ Close", callback_data="waifus_close"))
