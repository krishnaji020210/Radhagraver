...
async def gift_waifu(_, message):
    ...
    # ✅ If using /gift user_id waifu_id
    else:
        try:
    ...
    try:
        if message.chat.type == enums.ChatType.PRIVATE:
            ...
            await message.reply_text(
                "Your gift request has been sent to the receiver."
            )
        else:
            await message.reply_photo(
                photo=script.PHOTOS["GIFT_IMG"],
                caption=caption,
                reply_markup=buttons
            )
        ...
        await message.reply_text(
            "⚠️ <b>Couldn’t send gift. The user may have privacy settings enabled.</b>"
        )
